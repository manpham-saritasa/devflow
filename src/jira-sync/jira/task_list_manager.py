"""Task list management: pending, not-found, not-sync, force-sync."""

import json
from pathlib import Path

from jira.config import JIRA_PROJECT_KEY, PENDING_TASKS_PATH

RESOLVED_STATUSES = {"done", "completed", "resolved", "closed", "accepted", "canceled"}

NOT_SYNC_PATH = Path(__file__).resolve().parent.parent / "result" / "tasks-not-sync.txt"
FORCE_SYNC_PATH = (
    Path(__file__).resolve().parent.parent / "result" / "tasks-force-sync.txt"
)
